...
#    def __init__(self, in_channels, out_channels, encoder_name='resnet18', encoder_weights='imagenet'):
#        super(FPNUnet, self).__init__()
#        self.net = FPN(encoder_name=encoder_name, encoder_depth=3, encoder_weights=encoder_weights, decoder_pyramid_channels=256, decoder_segmentation_channels=128, decoder_merge_policy='add', decoder_dropout=0.2, in_channels=in_channels, classes=out_channels, activation='relu', upsampling=4)
#    
#    
#    def forward(self, x):
#        return self.net(x)
...
class VoxelMorph2d(nn.Module):
    ...
    def __init__(self, in_channels, use_gpu=True):
        ...
        self.localization = nn.Sequential(
             nn.Conv2d(in_channels, 4, kernel_size=7),
             nn.MaxPool2d(2, stride=2),
             nn.ReLU(True),
             nn.Conv2d(4, 6, kernel_size=5),
             nn.MaxPool2d(2, stride=2),
             nn.ReLU(True),
             nn.Conv2d(6, 10, kernel_size=7),
             nn.MaxPool2d(2, stride=2),
             nn.ReLU(True),
         )
...
#        diff = fixed_image - moving_image
        ...
        return registered_image, deformation_matrix, theta, moving_image#diff
