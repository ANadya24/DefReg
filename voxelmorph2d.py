...
class VoxelMorph2d(nn.Module):
    ...
    def forward(self, moving_image, fixed_image):
        # print("Im:", fixed_image.detach().cpu().numpy().max(), fixed_image.detach().cpu().numpy().min())
        # moving_image = self.stn(moving_image)
        # fixed_image = self.stn(fixed_image)
        ...
        registered_image = self.spatial_transform(moving_image.permute(0, 2, 3, 1),
                                                  deformation_matrix).permute(0, 3, 1, 2)
